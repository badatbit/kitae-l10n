...
|  |  |
|:--:|:--:|
| ![타이틀 메뉴](docs/images/0.png)<br>타이틀 메뉴 | ![인트로 텍스트 카드](docs/images/1.png)<br>인트로 텍스트 카드 |
| ![이름 입력 — 두벌식 한글 입력기](docs/images/2.png)<br>이름 입력 — 두벌식 한글 입력기 | ![본편 대사](docs/images/3.png)<br>본편 대사 |
| ![선택지](docs/images/4.png)<br>선택지 | ![거점 선택 — 지도 라벨](docs/images/5.png)<br>거점 선택 — 지도 라벨 |
| ![가이드북 지도](docs/images/6.png)<br>가이드북 지도 | ![노래방 곡 선택](docs/images/7.png)<br>노래방 곡 선택 |
...
| 이름 입력 화면 라벨·안내문·샘플 이름 74개 | — | 100% |
| 이름 입력 **한글 입력기**(두벌식 오토마타) | — | 동작 (인게임 확인) |
| 인트로 텍스트 카드 (`soz_001_00~03`) | 4장 | 100% |
| 동적 조사(받침 판정) | 5쌍 | 동작 |
| 글꼴 출력 | — | 가변폭 적용 |
...
- **이미지**
  - **타이틀 로고·저작권 이미지** `SOZ soz_008/009`
...
데이터 트랙은 **크기로 찾습니다** — GDI(`track03.bin`)든 Redump CUE/BIN(`… (Track 3).bin`)이든
이름이 달라도 됩니다. 두 덤프본의 데이터 트랙은 sha256 까지 같은 것을 확인했습니다(2026-09-25).

컴파일하려면 `dist/xdelta3.exe` 를 먼저 넣어야 합니다(GPL, 저장소에 두지 않습니다).
해시가 스크립트에 박히므로 **빌드할 때마다 생성기를 다시 돌려야** 합니다.

확인한 것(2026-09-25): NSIS 3.12 로 컴파일되고, Redump CUE/BIN 덤프본을 원본으로 무인 실행해
결과 데이터 트랙이 빌드본과 sha256 까지 일치했습니다. 해시를 일부러 틀리게 한 판으로는 `.dcp` 와
안내문이 출력 폴더에 떨어지는 것도 확인했습니다.
