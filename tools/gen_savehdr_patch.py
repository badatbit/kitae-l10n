...
`[월][月][' '][일][日][EN][시간대][EN][도시 8B(12px 공백 채움)][스폿 …30B]` 로 바꾼다.
숫자는 '０' 항목의 ko 를 `0`(ASCII 셀)으로 두어 엔진의 자릿수 조립이 ASCII 셀을 내고, 앞자리
패딩 '　' 항목은 FIGURE SPACE(U+2007, 폭 = 숫자 폭 16), 시간대 앞 공백은 12px(보호 항목은 U+3000=0x8140,
비보호는 EN 셀), 15칸 필드는 원문 그대로(0x8140 ×15 = 12px 씩).
  ...
  * HOOK C(0x10002e22, 12B): 시간대 끝(NUL 탐색) 뒤에 EN SPACE 셀 → 장소 서브루틴(r5=@(16,r8), r6=r9)
...
def stub_c(va):
    ...
    시간대와 장소 사이는 EN SPACE 셀(2바이트, 12px). base(0x10002f44)는 r12 에 보관한다
    (훅 뒤로 r11·r12 는 이 함수가 안 쓴다; 에필로그가 복원)."""
    base = 0x10002F44          # 장소함수 = base-12, 풀 = base+124 (둘 다 8비트 imm 안)
    L1 = va + 3 * 2
    em = _cell(chr(0x2002))            # 구분은 EN(12px) — EM 은 줄이 너무 길다(2026-09-19)
    body = [
        ...
        (S.mov_imm(_signed(em[0]), 0), None),   # 12 done: EN SPACE 셀[0]
        (0x2900, "mov.b r0,@r9"),          # 13
        (S.mov_imm(_signed(em[1]), 0), None),   # 14 EN SPACE 셀[1]
