...
반각 금지가 6번인 이유: 엔진은 타이밍 항목을 **줄마다 하나 + 2바이트 문자마다
하나** 소비한다. 반각은 항목을 하나도 안 먹는다. 그래서 반각을 H개 섞으면 배열이
그만큼 일찍 바닥나고, 남은 글자가 프레임당 하나씩 쏟아진다(줄 뒷부분이 날아간다).

    싱크 조건  H <= L - 1      L = 그 창의 줄 수

한 줄짜리 창은 반각을 하나도 못 쓴다. 여기서는 아예 금지한다 — 원문 23,030줄 중
반각을 그리는 줄은 39개뿐이고 그중 타이밍이 있는 줄은 0개다. 자세한 근거는
docs/MTG-TIMING.md.
"""
import io
import json
