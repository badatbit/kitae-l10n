...
def grid():
    """かな 격자에 놓을 90칸(행 우선 6행×15열, 5칸 블록 3개).

    **자음은 왼쪽 블록 4행, 모음은 가운데 블록 5행** — 모음이 블록을 넘어가면 찾기 어렵다
    (처음엔 가운데·오른쪽에 걸쳐 깔았다가 「ㅠ 가 없다」는 지적을 받았다, 9/24).
    모음 줄은 기본 모음마다 한 줄이다: ㅏ계 / ㅓ계 / ㅗ계 / ㅜ계 / ㅡ·ㅣ.
    """
    rows = [
        "ㄱㄲㄴㄷㄸ" + "ㅏㅐㅑㅒ " + " " * 5,
        "ㄹㅁㅂㅃㅅ" + "ㅓㅔㅕㅖ " + " " * 5,
