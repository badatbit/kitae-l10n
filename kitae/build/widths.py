...
## 폭을 어떻게 정하나
    ...
    합자           구성 글자 폭의 합
    `・`           22      말줄임(・・・)·나열 겸용, 칸 가운데
    `/`            12      반각 고정, 칸 가운데 — 가이드 지역 행·나열(보호 항목은 엔진 1바이트 글리프 12)
    `／` U+FF0F    24      선택지 구분 기호(시스템 설정 행) — 엔진이 24 고정 피치로 커서 상자를
                          두는 행에서 `/` 와 같은 모양을 24 칸 가운데에. 그 행은 공백도 EM, 숫자도 전각
                          (9/19 정책: ko 에서 `/` = 12px, `／` = 24px, 글리프는 같고 폭만 다르다)
...
DOT24 = "．"                 # 전각 마침표 = ASCII `.` 와 같은 모양을 **왼쪽 그대로** 두고 오른쪽을 비워 24 칸 (9/20 정책; 스탭롤 『북으로．』 — 원문 `。` 자리)
DIGITS = "0123456789"
DIGIT_PAD = 1                 # 숫자 고정폭 = 글꼴 전진폭 + 1 (오른쪽 끝 잘림 방지)
MARKUP_CHARS = "@&%*$"        # 제어 코드 문자 — 글자로 쓰려면 전각 ％＆＊＠＄
FONT_ADV = "²"                # ASCII 밖인데 글꼴 전진폭을 쓰는 라틴 기호 (km²)
SLASH_W = 12                  # ASCII `/` 는 반각 고정(칸 가운데) — `／` 24 와 같은 모양, 폭만 다르다 (9/19 정책)
...
# 키는 `{받침형무받침형}` 꼴 — **앞이 받침형**이다(와/과 는 관례와 반대인 `{과와}`). 본문과 절대 겹치지 않는다(`으로` 처럼 조사 철자를 그대로
# 쓰면 평범한 낱말과 부딪친다). 값은 (받침 있을 때, 없을 때). docs/JOSA-ENGINE.md
JOSA = {"{은는}": ("은", "는"), "{이가}": ("이", "가"), "{을를}": ("을", "를"),
        "{과와}": ("과", "와"), "{아야}": ("아", "야")}
# 길이가 다른 쌍((으)로·(이)라고·(이)야)은 한 셀에 못 담아 아직 못 쓴다 — docs/JOSA-ENGINE.md
