...
`(나)` 에서는 상자가 화면에 나오지 않는데 **타이머는 정상으로 돈다.** ２~３초
멈췄다가 기본값이 고른 것처럼 진행된다. 즉 시간 제한은 살아 있고 그리기만
빠진다. redream·Flycast 둘 다, **패치하지 않은 원본 디스크에서도** 같다
(2026-08-08 확인). 우리 번역과는 무관하고 에뮬레이션 쪽 구멍이다.

게이트 옵코드는 `<u16 대기 프레임> <u16 대상 ID>` 를 받는다(１２０·１８０·２６０
프레임, ID 0x22~0x45). 이걸 통째로 지우면 `(가)` 와 사실상 같은 형태가 된다.
